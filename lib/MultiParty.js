...
(function(global){
  ...
  MultiParty_.prototype.startMyStream_ = function() {
    ...
    navigator.getUserMedia_({"video": self.opts.video_stream, "audio": self.opts.audio_stream},
      function(stream) {
        ...
        if(window.AudioContext && window.AudioContext.prototype.createMediaStreamSource && window.AudioContext.prototype.createMediaStreamDestination){

          //Web Audio API対応で、かつcreateMediaStreamDestinationメソッドがある
          ...
          self.fire_('my_ms', {"src": URL.createObjectURL(self.stream), isUsingPlugin:false, "id": self.opts.id, "stream": self.stream});
          self.startCall_();
        }
        else {
          self.stream = stream;
          ...
          self.fire_('my_ms', {"src": self.stream, isUsingPlugin:true, "id": self.opts.id, "stream": self.stream});
          self.startCall_();
        }
  ...
  MultiParty_.prototype.setupPeerVideo_ = function(peer_id, stream, isReconnect) {
    ...
    if(stream.value){  //Using Plugin
      var src = stream;
      var isUsingPlugin = true;
    }
    else{
    ...
    this.fire_('peer_ms', {id: peer_id, src: src, isUsingPlugin:isUsingPlugin, reconnect: isReconnect});
  }
